...
def test_forced_encodings_with_df_mixed_types(library):
    ...
    # This is for testing py2 bytes vs str serialization issues. Ignoring Py3 for now.
    # ===================BEFORE===================
    df = pd.DataFrame(sample_data, index=['str_type', 'uni_type'])
    assert isinstance(df['str_col'][0], bytes)
    assert isinstance(df['unicode_col'][0], str)
    # Check that all column names are stored as is by pandas
    ...
    for index_val in df.index:
        if bytes(index_val) == 'uni_type':
            assert isinstance(index_val, str)
        else:
            assert isinstance(index_val, bytes)
    ...
    assert isinstance(df_normal['str_col'][0], bytes)
    assert isinstance(df_normal['unicode_col'][0], str)
    assert isinstance(s_str_normal.values[0], bytes)
    assert isinstance(s_unicode_normal.values[0], str)

    # Arctic currently converts all series names to bytes
    assert isinstance(s_str_normal.name, bytes)
    assert isinstance(s_unicode_normal.name, bytes)
    # Arctic currently converts all column and index string type names to str
    assert all(isinstance(x, str) for x in df_normal.columns)

    assert all(isinstance(x, str) for x in df_normal.index)
    assert all(isinstance(x, str) for x in s_str_normal.index)
    assert all(isinstance(x, str) for x in s_unicode_normal.index)
    ...
    assert isinstance(df_forced_unicode['str_col'][0], str)
    assert isinstance(df_forced_unicode['unicode_col'][0], str)
    assert isinstance(s_str_forced.values[0], str)
    assert isinstance(s_unicode_forced.values[0], str)

    # Arctic currently converts all column and index string type names to str
    assert isinstance(s_str_forced.name, str)
    assert isinstance(s_unicode_forced.name, str)
    assert all(isinstance(x, str) for x in df_forced_unicode.columns)

    assert all(isinstance(x, str) for x in df_forced_unicode.index)
    assert all(isinstance(x, str) for x in s_str_forced.index)
    assert all(isinstance(x, str) for x in df_forced_unicode.index)
...
def test_forced_encodings_with_df(library):
    ...
    assert all(isinstance(x, bytes) for x in df.columns)
    assert all(isinstance(x, bytes) for x in df.index)
    ...
    assert isinstance(s_str_normal.name, bytes)
    assert isinstance(s_unicode_normal.name, bytes)
    # Arctic currently converts all column to str and will keep index type as is
    assert all(isinstance(x, str) for x in df_normal.columns)

    assert all(isinstance(x, bytes) for x in df_normal.index)
    assert all(isinstance(x, bytes) for x in s_str_normal.index)
    assert all(isinstance(x, bytes) for x in s_unicode_normal.index)
    ...
    # Should force everything to be str now.
    assert isinstance(df_forced_unicode['str_col'][0], str)
    assert isinstance(df_forced_unicode['unicode_col'][0], str)
    assert isinstance(s_str_forced.values[0], str)
    assert isinstance(s_unicode_forced.values[0], str)
    ...
    assert all(isinstance(x, str) for x in df_forced_unicode.index)
    assert all(isinstance(x, str) for x in s_unicode_forced.index)
    assert all(isinstance(x, str) for x in s_str_forced.index)
...
def test_forced_encodings_with_df_py3(library):
    ...
    assert isinstance(df['str_col'][0], bytes)
    assert isinstance(df['unicode_col'][0], unicode_type)
    # Check that all column names are stored as as is by pandas
    assert all(isinstance(x, unicode_type) for x in df.columns)
    assert all(isinstance(x, bytes) for x in df.index)
    ...
    assert isinstance(df_normal['str_col'][0], bytes)
    assert isinstance(df_normal['unicode_col'][0], unicode_type)
    assert isinstance(s_str_normal.values[0], bytes)
    assert isinstance(s_unicode_normal.values[0], unicode_type)

    # Arctic currently converts all column to unicode_type and will keep index type as is
    assert isinstance(s_str_normal.name, unicode_type)
    assert isinstance(s_unicode_normal.name, unicode_type)
    assert all(isinstance(x, unicode_type) for x in df_normal.columns)
    ...
    assert isinstance(df_forced_unicode['str_col'][0], unicode_type)
    assert isinstance(df_forced_unicode['unicode_col'][0], unicode_type)
    assert isinstance(s_str_forced.values[0], unicode_type)
    assert isinstance(s_unicode_forced.values[0], unicode_type)

    # Should force everything to be unicode_type now.
    assert isinstance(s_str_forced.name, unicode_type)
    assert isinstance(s_unicode_forced.name, unicode_type)
    assert all(isinstance(x, unicode_type) for x in df_forced_unicode.columns)

    assert all(isinstance(x, unicode_type) for x in df_forced_unicode.index)
    assert all(isinstance(x, unicode_type) for x in s_str_forced.index)
    assert all(isinstance(x, unicode_type) for x in s_unicode_forced.index)
...
def test_forced_encodings_with_df_py3_multi_index(library):
    ...
    def _assert_index_type(index, t_type):
        assert all(
            isinstance(index.get_level_values(level)[0], t_type)
    ...
    assert isinstance(multi_index_df['str_col'][0], bytes)
    assert isinstance(multi_index_df['unicode_col'][0], unicode_type)
    # Check that all column names are stored as as is by pandas
    assert all(isinstance(x, unicode_type) for x in multi_index_df.columns)
    assert all(
        isinstance(multi_index_df.index.get_level_values(level)[0], bytes)
    ...
    assert isinstance(s_unicode_col.values[0], unicode_type)

    # Arctic currently converts all column to unicode_type and will keep index type as is
    assert isinstance(s_str_col.name, unicode_type)
    assert isinstance(s_unicode_col.name, unicode_type)
    assert all(isinstance(x, unicode_type) for x in df_normal.columns)
